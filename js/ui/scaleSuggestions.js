 ...
 */
export function renderScaleSuggestions(activeKey, tuning, onActivate, progression = []) {
  ...
  const topCardsHtml = suggestions.map(({ type, reason }) => {
    return _buildScaleCard(type, reason, activeKey, tuning, progression);
  }).filter(Boolean).join('');
  ...
  const restCardsHtml = restTypes.map(type => {
    return _buildScaleCard(type, '', activeKey, tuning, progression, true);
  }).filter(Boolean).join('');
  ...
  container.querySelector('#ssc-show-all-btn')?.addEventListener('click', () => {
    _showAll = !_showAll;
    renderScaleSuggestions(activeKey, tuning, onActivate, progression);
  });
}

function _buildScaleCard(type, reason, activeKey, tuning, progression, compact = false) {
  const positions = getScalePositions(activeKey.root, type, tuning);
  ...
  if (progression.length > 0) {
    ...
    fitBadgesHtml = `
      <div class="chord-fit-row">
        ${chordFits.map(({ name, fits }) => `
          <span class="chord-fit-badge chord-fit-${fits}" title="${fits}">${name}</span>
        `).join('')}
      </div>
    `;
  }
...
function _handleCardClick(e, activeKey, tuning, onActivate, progression) {
